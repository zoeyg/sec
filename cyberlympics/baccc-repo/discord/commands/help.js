...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (!args.length) {
      ...
      return message.author
        ...
        .catch((err) => {
          ...
          message.reply("I tried to DM you with the commands, but something went wrong. Did you disable DM's?");
        });
    }
    ...
    if (!command) {
      return message.reply(`\`${name}\` is not one of the recognized commands.`);
    }
    ...
    if (command.permissions && command.permissions.length) {
      data.push(
        `**Required Permissions:** ${command.permissions
          .map((permission) => `\`${enumToString(discord.Permissions.FLAGS, permission)}\``)
          .join(", ")}`
      );
    }
    if (command.usage) {
