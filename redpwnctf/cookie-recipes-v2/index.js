...
const util = {
    ...
    parseRequest: async (req) => {
        return await new Promise((resolve, reject) => {
            ...
            req.on('end', () => {
                try {
                    let theBuffer = Buffer.concat(chunks);
                    console.log("Body:", theBuffer.toString('utf-8'));
                    let theJson = JSON.parse(theBuffer);
                    console.log(theJson);
                    resolve(theJson);
                } catch (e) {
                    reject(e);
...
if (!database.hasUser('admin')) {
    // Admin gets id 0
    database.register('admin', 'n3cdD3GjyjGUS8PZ3n7dvZerWiY9IRQn', 1, '0', 'admintoken');
