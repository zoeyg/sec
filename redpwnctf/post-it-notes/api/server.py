...
def get_note(nid):
    print("NID IS \"" + nid + "\"")
    cmd = f"cat 'notes/{nid}' || echo it did not work btw"
    print("CMD IS \"" + cmd + "\"")
    stdout, stderr = subprocess.Popen(cmd, shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE, stdin = subprocess.PIPE).communicate()
    if stderr:
